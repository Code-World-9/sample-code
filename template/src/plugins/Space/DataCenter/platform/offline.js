...
const dataCenter = {
	...
	async init(workId) {
		...
		cache.setting = replacer.execute(
			await offlineServer.getSetting(workId),
			{ origin: offlineServer.baseUrl }
		);
		cache.initial = true;
	},
	...
	getSceneBarList() {
		...
		return Adapter.sceneGroup.adapt('offline-1.0.0', 'sceneGroup-1.3.0', { scenes: setting.scenes });
	},
	...
	getHotspotTagData(sceneId) {
		...
		return Adapter.hotspotTag.adapt('offline-1.0.0', 'HotspotTagLoadOptions-1.3.0', {
			scene,
